...
public class GudidIStatedDefinitionSemanticIT extends AbstractIntegrationTest {
     ...
     *
     * @result Reads content from file and validates Axiom of Semantics by calling private method assertLine().
     */
    @Test
    public void testGudidIStatedDefinitionSemantics() throws IOException {
...
//        String absolutePath = gudIdFileName; //Unable to find 2932991 device.txt semantics. Details written to target/failsafe-reports/gudid_concepts_not_found.txt ==> expected: <0> but was: <2932991>
        String absolutePath = findFilePath(sourceFilePath, "productCodes.txt"); //Unable to find 6987 productCodes.txt.txt semantics. Details written to target/failsafe-reports/gudid_concepts_not_found.txt ==> expected: <0> but was: <6987>
        int notFound = processFile(absolutePath, errorFile);

        assertEquals(0, notFound, "Unable to find " + notFound + " Gudid Axiom semantics for Devices. Details written to " + errorFile);
    }
